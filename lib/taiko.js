...
function getEventProxy(target) {
    let unsupprtedCleintMethods = ['removeListener', 'emit', 'removeAllListeners', 'setMaxListeners', 'off'];
    const handler = {
        get: (target, name) => {
            if (unsupprtedCleintMethods.includes(name)) throw new Error(`Unsupported action ${name} on client`);
            return target[name];
        }
    };
 ...
 *
 * @param {string} - Element to wait
 * @param {number|time} - Time to wait. default to 10s
 * @return {promise}
 */
...
const waitFor = async(element, time) => {
    ...
    if(!element || isFinite(element)) {
        time = element;
        wait(time);
    } else if (isSelector(element) || isString(element)){
